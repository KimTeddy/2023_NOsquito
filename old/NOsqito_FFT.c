...
float ADC_buffer[300];
float FFT_buffer[2048], FFT_input[2048], FFT_output[1024], max_value;
...
int main(void)
{
  ...
  arm_cfft_radix4_init_f32(&S, 1024, 0, 1);      // initialize radix-4 CFFT function
  ...
  while(1)
    {
	  ...
	  for (i = 0; i < 2048; i++)
          ...
          arm_max_f32(FFT_output, 1024, &max_value, &max_index);

          for (i = 1; i < 128; i++)		// draw FFT result
            Draw_FFT(i, FFT_output[i+5 ]*2);
        }
    }
}
